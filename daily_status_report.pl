...
sub generate_html_report_linux_hosts {
   ...
   foreach $key (sort keys %linux_hosts) {
      ...
      foreach $key2 (keys(%{$linux_hosts{$key}{linux_fs}})) {						#$keys2 loops through each hrStorageIndex hash key
         ...
         next if ( ($linux_hosts{$key}{linux_fs}{$key2}{hrStorageDescr} =~ /^\/var\/lib\/docker\/overlay2\//)          && ($linux_hosts{$key}{linux_fs}{$key2}{hrStorageUsed_pct} <= 90) );       #skip any filesystems used by containers that are less than 90% full, just because there may be dozens or hundreds
